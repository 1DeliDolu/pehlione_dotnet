...
public sealed class AccountController : Controller
{
    ...
    public async Task<IActionResult> Login(LoginViewModel model)
    {
        ...
        if (result.Succeeded)
        {
            ...
            var forceRoleDashboard = await _userManager.IsInRoleAsync(user, IdentitySeed.RoleAdmin)
                ...
                || await _userManager.IsInRoleAsync(user, IdentitySeed.RoleCourier);
    ...
    private async Task<IActionResult> RedirectToRoleDashboardAsync(ApplicationUser user)
    {
        ...
        if (await _userManager.IsInRoleAsync(user, IdentitySeed.RoleCourier))
            return RedirectToAction("Index", "Courier", new { area = "Staff" });
